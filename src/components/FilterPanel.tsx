...
const PROPERTY_TYPES = ['Departamento', 'Casa', 'PH', 'Lote'];
...
export function FilterPanel({
  ...
  const { toggleArray, toggleBoolean, handleClear, activeCount, setOperationType, setSellerType, setCurrency, setParking } = useFilterPanel(
    filter,
    onFilterChange,
    onClose
  );
            ...
            <div className="flex flex-col">
              ...
              <div className="space-y-6 px-5 py-5">
                        ...
                        <motion.button
                          ...
                          onClick={() =>
                            onFilterChange({
                              ...filter,
                              sellerType:
                                filter.sellerType === option.value
                                  ? undefined
                                  : option.value,
                            })
                          }
                ...
                <section className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex rounded-full border border-border-subtle bg-card p-1">
                      {CURRENCY_OPTIONS.map((option) => {
                        const isActive =
                          filter.currency === option.value ||
                          (option.value === 'any' && !filter.currency);
                        return (
                          <button
                            key={option.value}
                            onClick={() =>
                  ...
                  <div className="flex items-center justify-between rounded-2xl border border-border-subtle bg-app px-4 py-3">
                ...
                <section className="space-y-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {ROOMS_OPTIONS.map((item) => {
                        const isActive = filter.rooms?.includes(item.value);
                        return (
                          <motion.button
                            key={item.label}
                            onClick={() => toggleArray('rooms', item.value)}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {BEDROOMS_OPTIONS.map((item) => {
                        const isActive = filter.bedrooms?.includes(item.value);
                        return (
                          <motion.button
                            key={item.label}
                            onClick={() => toggleArray('bedrooms', item.value)}
                            className={`h-10 min-w-[3rem] rounded-full border px-3 text-sm transition-all ${
                              isActive
                                ? 'border-content-primary bg-brand-olive text-white'
                                : 'border-border-subtle bg-card text-content-secondary hover:border-content-primary/40'
                ...
                <section className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {AMENITY_OPTIONS.map((amenity) => {
                      ...
                      const isActive = filter.amenities?.some(
